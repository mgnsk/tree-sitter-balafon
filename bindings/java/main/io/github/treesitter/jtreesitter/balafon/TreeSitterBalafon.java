package io.github.treesitter.jtreesitter.balafon;

import java.lang.foreign.*;

public final class TreeSitterBalafon {
    ...
    private static final TreeSitterBalafon INSTANCE = new TreeSitterBalafon();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterBalafon() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_balafon");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-balafon");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-balafon");
