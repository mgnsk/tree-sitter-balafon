const terminator = choice(";", "\n"),
  ...
  cmdKey = seq(prefix, "key"),
  cmdTime = seq(prefix, "time"),
  cmdVelocity = seq(prefix, "velocity"),
  cmdChannel = seq(prefix, "channel"),
  cmdVoice = seq(prefix, "voice"),
  ...
  rules: {
    ...
    command: ($) =>
      choice(
        ...
        seq(cmdKey, field("scale", $.ident)),
        seq(cmdTime, field("num", $.uint), field("denom", $.uint)),
        seq(cmdVelocity, field("velocity", $.uint)),
        seq(cmdChannel, field("channel", $.uint)),
        seq(cmdVoice, field("voice", $.uint)),
